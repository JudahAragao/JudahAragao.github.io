...
export default function Works({ dataEducationsAndWorks }: CardEducationsAndWorksProps) {
  return <>
    {
        ...
        return <Card key={index} className='w-[443.5px]'>
          ...
          <CardContent className="px-6 pb-6 text-sm">
            <ul className="list-disc pl-5">
              {
                data.responsibilities && data.responsibilities?.map((description, index) => {
                  return <li key={index}>{description.description}</li>
                })
              }
            </ul>
          </CardContent>
